...
/*   Updated: 2023/04/03 20:21:10 by nibenoit         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	forks(t_list *commands, char **envp, int fd_rw[2], int fd_pipe[2])
{
	t_command	*cmd;
	char		*pathname;
	int			last_pid;
	...
	while (commands)
	{
		...
		last_pid = exec(pathname, cmd->args, envp, fd_rw, fd_pipe[0]);
		free(pathname);
...
int	pipex(t_list *commands, char **envp)
{
	...
	last_pid = forks(commands, envp, fd_rw, fd_pipe);
	dprintf(1, "last_pid = %d\n", last_pid);
